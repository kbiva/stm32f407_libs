...
static PIN pins[]={
  {{XPT2046_SCK_Pin,XPT2046_SCK_Mode,XPT2046_SCK_Speed,XPT2046_SCK_OType,XPT2046_SCK_PuPd},XPT2046_SCK_Port,XPT2046_SCK_Bus,XPT2046_SCK_PinSource},
  {{XPT2046_MISO_Pin,XPT2046_MISO_Mode,XPT2046_MISO_Speed,XPT2046_MISO_OType,XPT2046_MISO_PuPd},XPT2046_MISO_Port,XPT2046_MISO_Bus,XPT2046_MISO_PinSource},
  {{XPT2046_MOSI_Pin,XPT2046_MOSI_Mode,XPT2046_MOSI_Speed,XPT2046_MOSI_OType,XPT2046_MOSI_PuPd},XPT2046_MOSI_Port,XPT2046_MOSI_Bus,XPT2046_MOSI_PinSource},
  {{XPT2046_CS_Pin,XPT2046_CS_Mode,XPT2046_CS_Speed,XPT2046_CS_OType,XPT2046_CS_PuPd},XPT2046_CS_Port,XPT2046_CS_Bus,XPT2046_CS_PinSource},
  {{XPT2046_IRQ_Pin,XPT2046_IRQ_Mode,XPT2046_IRQ_Speed,XPT2046_IRQ_OType,XPT2046_IRQ_PuPd},XPT2046_IRQ_Port,XPT2046_IRQ_Bus,XPT2046_IRQ_PinSource},
};
...
static void GPIO_Configuration(void) {
  ...
  for(i=0;i<sizeof(pins)/sizeof(PIN);i++) {
    ...
    if(pins[i].GPIO_InitStructure.GPIO_Mode==GPIO_Mode_AF)
      GPIO_PinAFConfig(pins[i].GPIOx,pins[i].GPIO_PinSource,GPIO_AF_SPI2);
  }
}
...
void XPT2046_ILI9325_Init(void) {
  ...
  SYSCFG_EXTILineConfig(XPT2046_EXTI_Portsource, XPT2046_IRQ_PinSource);
...
static uint8_t XPT2046_send(uint8_t cmd) {
  ...
  while (!(SPI2->SR & SPI_I2S_FLAG_TXE)){}; //while (SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_TXE) == RESET);
  SPI2->DR=cmd;                             //SPI_I2S_SendData(SPI2,cmd);
  while (!(SPI2->SR & SPI_I2S_FLAG_RXNE)){};//while (SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_RXNE) == RESET);
  return SPI2->DR;                          //return SPI_I2S_ReceiveData(SPI2);
}
...
static uint8_t getUserInput(
  ...
  *panelPointX=(ILI9325_GetWidth()*xpercent)/100;
  *panelPointY=(ILI9325_GetHeight()*ypercent)/100;

  // display the hit-point for the user
  ILI9325_ClearScreen(BLACK);
  ILI9325_PutStrCEOL("Tap and hold the red point",44,10);
  ...
  ILI9325_PutStrCEOL("Stop pressing the screen",44,10);
